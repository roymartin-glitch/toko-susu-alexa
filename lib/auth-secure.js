// Edge Runtime compatible — NO Node.js crypto / bcrypt
...
const ADMIN_USERS = [
  {
    id: '1',
    email: 'tokosusualexa@gmail.com',
    passwordPlain: 'Jean2024',
    name: 'Administrator',
    role: 'admin'
  },
  {
    id: '2',
    email: 'admin@kasgo.com',
...
async function getKey() {
  ...
  return await crypto.subtle.importKey('raw', enc.encode(JWT_SECRET), { name: 'HMAC', hash: 'SHA-256' }, false, ['sign', 'verify'])
}
...
async function verifySignature(data, signature) {
  const key = await getKey()
  const enc = new TextEncoder()
  const padded = signature + '=='.substring(0, (4 - (signature.length % 4)) % 4)
...
export async function generateToken(payload) {
  ...
  const claims = { ...payload, iat: now, exp: now + Math.floor(TOKEN_EXPIRY / 1000) }
  const headerEncoded = encodeBase64url(JSON.stringify(header))
  const claimsEncoded = encodeBase64url(JSON.stringify(claims))
  const sigEncoded = await sign(`${headerEncoded}.${claimsEncoded}`)
...
export async function verifyToken(token) {
  try {
    ...
    const valid = await verifySignature(`${headerEncoded}.${claimsEncoded}`, sigEncoded)

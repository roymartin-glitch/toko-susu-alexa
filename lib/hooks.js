...
export const useCreateTransaction = () => {
  ...
  return useMutation({
    mutationFn: async (transaction) => {
      // Pre-flight: Check session before attempting transaction
      const { ensureValidSession } = require('./supabase')
      const sessionCheck = await ensureValidSession(5) // Check if < 5 min left
      
      if (!sessionCheck.success) {
        console.error('❌ Session invalid before transaction:', sessionCheck.error)

...
export const transactionsApi = {
  ...
  create: async (transaction) => {
    // ✅ STEP 1: ENSURE VALID SESSION BEFORE MAKING REQUEST
    // Check session and refresh if needed (within 10 minutes of expiry)
    const sessionCheck = await ensureValidSession(10)
    ...
    console.log('✅ Session validated, proceeding with transaction')
    
    // ✅ STEP 2: MAKE REQUEST WITH AUTOMATIC RETRY ON 401
    // Using the new retryWithAuth utility for robust retry logic
    ...
    try {
      // Retry up to 2 times (initial + 1 retry) with 500ms delay
      const result = await retryWithAuth(makeTransactionRequest, 1, 500)
      ...
      if (error.message.includes('login') || error.message.includes('sesi')) {
        throw error // Keep auth-related errors as-is
      }
